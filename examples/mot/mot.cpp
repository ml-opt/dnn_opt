...
int main(int argc, char** argv)
{
  ...
  int k = input("-k", 2, argc, argv);
  float beta = input_f("-beta", 0.2f, argc, argv);
  ...
  auto* mot = algorithms::mot::make(algorithm, train, k, beta);
  ...
  terror = dynamic_cast<solutions::network*>(mot->get_best())->test(train);
  gerror = dynamic_cast<solutions::network*>(mot->get_best())->test(test);
